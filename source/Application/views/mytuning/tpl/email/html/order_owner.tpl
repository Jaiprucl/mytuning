...
[{block name="email_html_order_owner_address"}]
    <!-- Address info -->
    <table class="row footer" cellspacing="1" cellpadding="1" width="100%">
        <tbody>
        <tr>
...
<table class="orderarticles" border="0" cellspacing="1" cellpadding="1" width="100%">
    <thead>
        <tr bgcolor="#E68D00">
        ...
        [{foreach key=basketindex from=$basket->getContents() item=basketitem}]
            [{block name="email_html_order_owner_basketitem"}]
                ...
                <tr valign="top">
                    ...
                    <td><p>[{$basketitem->getAmount()}]</p></td>
    ...
    <tfoot>
        ...
        [{if !$basket->getDiscounts()}]
            ...
            [{block name="email_html_order_owner_nodiscounttotalgross"}]
                <!-- brutto price -->
                <tr valign="top">
                    <td width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{oxmultilang ident="TOTAL_GROSS"}]</td>
                    <td width="40%" align="right" class="odd text-right">[{$basket->getFProductsPrice()}] [{$currency->sign}]</td>
                </tr>
            [{/block}]
        [{/if}]
        ...
        [{if $basket->getDiscounts()}]
            [{if $order->isNettoMode()}]
                ...
                <!-- netto price -->
                    <tr valign="top">
                        <td width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{oxmultilang ident="TOTAL_NET"}]</td>
                        <td width="40%" align="right" class="odd text-right">[{$basket->getProductsNetPrice()}] [{$currency->sign}]</td>0
                    </tr>
                [{/block}]
            [{else}]
            ...
            [{block name="email_html_order_owner_discounts"}]
                ...
                [{foreach from=$basket->getDiscounts() item=oDiscount}]
                    <tr valign="top">
                        <td width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{if $oDiscount->dDiscount < 0}][{oxmultilang ident="SURCHARGE"}][{else}][{oxmultilang ident="DISCOUNT"}][{/if}] <em>[{$oDiscount->sDiscount}]</em>:</td>
                        <td width="40%" align="right" class="odd text-right">[{if $oDiscount->dDiscount < 0}][{$oDiscount->fDiscount|replace:"-":""}][{else}]-[{$oDiscount->fDiscount}][{/if}] [{$currency->sign}]</td>
                    </tr>
                [{/foreach}]
            [{/block}]
