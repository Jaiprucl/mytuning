...
[{block name="email_html_order_cust_address"}]
    <!-- Address info -->
    <table class="row footer" width="100%" style="width:100%" cellspacing="1" cellpadding="1" width="100%">
        <tbody>
            <tr>
...
<table class="orderarticles" width="100%" style="width:100%;" cellspacing="1" cellpadding="1" width="100%">
    <thead>
        <tr bgcolor="#E68D00">
            <th style="padding: 5px;" width="60%" [{if $oViewConf->getViewThemeParam('blEmailsShowProductPictures')}] colspan="2"[{/if}]>[{oxmultilang ident="PRODUCT"}]</th>
            <th style="padding: 5px;" width="15%" >[{oxmultilang ident="UNIT_PRICE"}]</th>
            <th style="padding: 5px;" width="10%" >[{oxmultilang ident="QUANTITY"}]</th>
            <!--<th>[{oxmultilang ident="VAT"}]</th>-->
            <th style="padding: 5px;" width="15%" >[{oxmultilang ident="TOTAL"}]</th>
        </tr>
    </thead>
    <tbody>
    ...
    <tfoot>
        ...
        [{if !$basket->getDiscounts()}]
            ...
            [{block name="email_html_order_cust_nodiscounttotalgross"}]
                <!-- brutto price -->
                <tr valign="top">
                    <td align="right" width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{oxmultilang ident="TOTAL_GROSS"}]</td>
                    <td align="right" width="40%" class="odd text-right">[{$basket->getFProductsPrice()}] [{$currency->sign}]</td>
                </tr>
            [{/block}]
        [{/if}]
        <!-- applied discounts -->
        [{if $basket->getDiscounts()}]
            [{if $order->isNettoMode()}]
                [{block name="email_html_order_cust_discounttotalnet"}]
                    <!-- netto price -->
                    <tr valign="top">
                        <td align="right" width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{oxmultilang ident="TOTAL_NET"}]</td>
                        <td align="right" width="40%" class="odd text-right">[{$basket->getProductsNetPrice()}] [{$currency->sign}]</td>
                    </tr>
                [{/block}]
            [{else}]
            ...
            [{block name="email_html_order_cust_discounts"}]
                ...
                [{foreach from=$basket->getDiscounts() item=oDiscount}]
                    <tr valign="top">
                        <td align="right" width="60%" colspan="[{$iFooterColspan}]" class="odd text-right">[{if $oDiscount->dDiscount < 0}][{oxmultilang ident="SURCHARGE"}][{else}][{oxmultilang ident="DISCOUNT"}][{/if}] <em>[{$oDiscount->sDiscount}]</em> :</td>
                        <td align="right" width="40%" class="odd text-right">[{if $oDiscount->dDiscount < 0}][{$oDiscount->fDiscount|replace:"-":""}][{else}]-[{$oDiscount->fDiscount}][{/if}] [{$currency->sign}]</td>
                    </tr>
                [{/foreach}]
            [{/block}]
...
<table width="100%">
    <tr>
        <td align="right">
            <p>Rechnungsstellung erfolgt ohne Ausweis der Umsatzsteuer nach §19 UStG</p>
        </td>
    </tr>
</table>
<br />
