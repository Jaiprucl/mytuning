...
class hd_file extends AdminController
{
     ...
     * @var string
     */
    protected $_sThisTemplate = 'hd_file.tpl';
    ...
    public function listFolderFilesSub($dir)
    {
        ...
        foreach ($ffs as $ff) {
            if ($ff != '.' && $ff != '..') {
                if (!is_dir($dir . '/' . $ff)) {
                    $out .= '<li><span class="file"><a href=" ../modules/hdnet/hd_admin/file.php?get=' . ltrim($dir . '/' . $ff, './') . '">' . $ff . '</a><span>';
                } else {
